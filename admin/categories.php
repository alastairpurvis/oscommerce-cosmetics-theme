...
/*
  ...
  Portions Copyright (c) 2008 Club osCommerce www.clubosc.com
...
// removing categories can be a lengthy process
      ...
      case 'update_product':
        if (isset($HTTP_POST_VARS['edit_x']) || isset($HTTP_POST_VARS['edit_y'])) {
          ...
          if (isset($HTTP_POST_VARS['large_products_image']) && tep_not_null($HTTP_POST_VARS['large_products_image']) && ($HTTP_POST_VARS['large_products_image'] != 'none')) {
            $sql_data_array['large_products_image'] = tep_db_prepare_input($HTTP_POST_VARS['large_products_image']);
          }
      ...
      case 'copy_to_confirm':
        if (isset($HTTP_POST_VARS['products_id']) && isset($HTTP_POST_VARS['categories_id'])) {
          ...
          if ($HTTP_POST_VARS['copy_as'] == 'link') {
            ...
            $product_query = tep_db_query("select products_quantity, products_model, products_image, large_products_image, products_price, products_date_available, products_weight, products_tax_class_id, manufacturers_id from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
            $product = tep_db_fetch_array($product_query);

            tep_db_query("insert into " . TABLE_PRODUCTS . " (products_quantity, products_model,products_image, large_products_image, products_price, products_date_added, products_date_available, products_weight, products_status, products_tax_class_id, manufacturers_id) values ('" . tep_db_input($product['products_quantity']) . "', '" . tep_db_input($product['products_model']) . "', '" . tep_db_input($product['products_image']) . "', '" . tep_db_input($product['large_products_image']) . "', '" . tep_db_input($product['products_price']) . "',  now(), " . (empty($product['products_date_available']) ? "null" : "'" . tep_db_input($product['products_date_available']) . "'") . ", '" . tep_db_input($product['products_weight']) . "', '0', '" . (int)$product['products_tax_class_id'] . "', '" . (int)$product['manufacturers_id'] . "')");
...
// copy LARGE image only if modified
        $large_products_image = new upload('large_products_image');
        $large_products_image->set_destination(DIR_FS_CATALOG_IMAGES);
        if ($large_products_image->parse() && $large_products_image->save()) {
          $large_products_image_name = $large_products_image->filename;
        } else {
          $large_products_image_name = (isset($HTTP_POST_VARS['large_products_previous_image']) ? $HTTP_POST_VARS['large_products_previous_image'] : '');
        }
        break;
    }
  }
...
?>
          ...
          <tr>
            <td class="main">Skin Conditions</td>
          ...
          <tr>
            <td class="main"><?php echo tep_draw_hidden_field('products_previous_image', $pInfo->products_image); ?></td>
          </tr>
          <tr>
            <td class="main"><?php echo tep_draw_hidden_field('large_products_previous_image', $pInfo->large_products_image); ?></td>
          </tr>
...
<?php
    ...
    if (tep_not_null($HTTP_POST_VARS)) {
      ...
      $product_query = tep_db_query("select p.products_id, pd.language_id, pd.products_name, pd.products_description, pd.products_url, p.products_quantity, p.products_model, p.products_image, p.large_products_image, p.products_price, p.products_weight, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p.manufacturers_id, p.products_ship_sep  from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "'");
      $product = tep_db_fetch_array($product_query);

      $pInfo = new objectInfo($product);
      $products_image_name = $pInfo->products_image;
      $large_products_image_name = $pInfo->large_products_image;
    }
...
/* Re-Post all POST'ed variables */
      ...
      echo tep_draw_hidden_field('large_products_image', stripslashes($large_products_image_name));
...
<?php
    ...
    $products_count = 0;
    if (isset($HTTP_GET_VARS['search'])) {
      $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_quantity, p.products_image, p.large_products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status, p2c.categories_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and pd.products_name like '%" . tep_db_input($search) . "%' order by pd.products_name");
    } else {
      $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_quantity, p.products_image, p.large_products_image, p.products_price, p.products_date_added, p.products_last_modified, p.products_date_available, p.products_status from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd, " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c where p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "' and p.products_id = p2c.products_id and p2c.categories_id = '" . (int)$current_category_id . "' order by pd.products_name");
    }

<?php
//Modified by pektsekye@gmail.com on Monday April 26 2009
 ...
 */	
	function href_link($page = '', $parameters = '', $connection = 'NONSSL', $add_session_id = false){
		...
		if ( !in_array($page, $this->attributes['SEO_PAGES']) || $this->attributes['SEO_ENABLED'] == 'false' ) {
			          $badword = ".php";
	        $len = strlen($badword);
					$page = preg_replace("/\b$badword\b/", '', $page); 
				...
				case 'products_id':
					...
					switch(true){
						...
						case ( $page == FILENAME_PRODUCT_REVIEWS ):
							$url = $this->make_url($page, $this->get_product_name($p2[1]), 'products_id_review', $p2[1], '', $separator);
                            $this->ValidateName($url, "pr", $p2[1], $connection, $separator);
							break;
						case ( $page == FILENAME_PRODUCT_REVIEWS_INFO ):
							$url = $this->make_url($page, $this->get_product_name($p2[1]), 'products_id_review_info', $p2[1], '', $separator);
				...
				case 'authors_id':
					switch(true){
						case ($page == FILENAME_ARTICLES):
							$url = $this->make_url($page, $this->get_authors_name($p2[1]), $p2[0], $p2[1], '', $separator);
				...
				case 'lPath':
					switch(true){
						case ($page == FILENAME_LINKS):
							$url = $this->make_url($page, $this->get_link_name($p2[1]), $p2[0], $p2[1], '', $separator);
				...
				case 'articles_id':
					switch(true){
						case ($page == FILENAME_ARTICLE_INFO):
							$url = $this->make_url($page, $this->get_article_name($p2[1]), $p2[0], $p2[1], '', $separator);
				...
				case 'info_id':
					switch(true){
						case ($page == FILENAME_INFORMATION):
							$url = $this->make_url($page, $this->get_information_name($p2[1]), $p2[0], $p2[1], '', $separator);
				...
				case 'faqdesk_id':
                                        switch(true){
                                                case ($page == FILENAME_FAQDESK_INFO):
                                                        $url = $this->make_url($page, $this->get_faqdesk_name($p2[1]), $p2[0], $p2[1], '', $separator);
                                                        break;
												case ($page == FILENAME_FAQDESK_REVIEWS_INFO):
                                                        $url = $this->make_url($page, $this->get_faqdesk_name($p2[1]), 'faqdesk_reviews_id', $p2[1], '', $separator);
                                                        break;
												case ($page == FILENAME_FAQDESK_REVIEWS_ARTICLE):
                                                        $url = $this->make_url($page, $this->get_faqdesk_name($p2[1]), 'faqdesk_article_id', $p2[1], '', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
				...
				case 'faqPath':
                                        switch(true){
                                                case ($page == FILENAME_FAQDESK_INDEX):
                                                        $url = $this->make_url($page, $this->get_faqdesk_categories_name($p2[1]), $p2[0], $p2[1], '', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
				...
				case 'pages_id':
					switch(true){
						case ($page == FILENAME_PAGES):
							$url = $this->make_url($page, $this->get_pages_name($p2[1]), $p2[0], $p2[1], '', $separator);
							break;
						default: 
							$container[$p2[0]] = $p2[1];
							break;
...
case 'pollid':
                                        switch(true){
                                                case ($page == 'pollbooth.php'):
                                                        $url = $this->make_url($page, $this->get_polls_name($p2[1]), $p2[0], $p2[1], '', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
				...
				case 'newsdesk_id':
                                        switch(true){
                                                case ($page == FILENAME_NEWSDESK_INFO):
                                                        $url = $this->make_url($page, $this->get_newsdesk_name($p2[1]), $p2[0], $p2[1], '', $separator);
                                                        break;
												case ($page == FILENAME_NEWSDESK_REVIEWS_INFO):
                                                        $url = $this->make_url($page, $this->get_newsdesk_name($p2[1]), 'newsdesk_reviews_id', $p2[1], '', $separator);
                                                        break;
												case ($page == FILENAME_NEWSDESK_REVIEWS_ARTICLE):
                                                        $url = $this->make_url($page, $this->get_newsdesk_name($p2[1]), 'newsdesk_article_id', $p2[1], '', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
				...
				case 'newsPath':
                                        switch(true){
                                                case ($page == FILENAME_NEWSDESK_INDEX):
                                                        $url = $this->make_url($page, $this->get_newsdesk_categories_name($p2[1]), $p2[0], $p2[1], '', $separator);
                                                        break;
                                                default: 
                                                        $container[$p2[0]] = $p2[1];
                                                        break;
 ...
 * @return string
 */	
	function make_url($page, $string, $anchor_type, $id, $extension = '', &$separator){
 ...
 */	
	function do_redirect(){
		...
		$separator = '?';
		$url = $this->base_url . $this->parse_parameters($this->uri_parsed['path'], $params, $separator);	

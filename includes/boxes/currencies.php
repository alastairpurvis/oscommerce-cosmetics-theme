...
<?php
  ...
  $keys = Array('currency',tep_session_name(),'x','y');
  if (SEO_ENABLED == 'true' && basename($PHP_SELF) != FILENAME_ADVANCED_SEARCH_RESULT){ 
	$keys [] = 'cPath';
	$keys [] = 'products_id';	
  ...
  while (list($key, $value) = each($HTTP_GET_VARS)) {
    ...
    $info_box_contents = array();
    $info_box_contents[] = array('form' => tep_draw_form('currencies', tep_href_link(basename($PHP_SELF), tep_get_all_get_params(), $request_type, false), 'get'),
                                 'align' => 'center',
